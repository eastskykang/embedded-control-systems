...
void IsrA(void)
{
    ...
    // scale input 
    an_in0_scaled = (0.9 - 0.1) / (16060 - 120) * (an_in0 - 120) + 0.1; 

    if (SIU.GPDI[DIP_PIN].B.PDI) {
        // high 
...
void IsrB(void)
{
    ...
    // read ANin0 
    sampled_sin = sin(2*PI*i_B/10);

    //sampled_sin = sinf(2*PI*i_B/10);
    // scale input 
...
int main() {
    ...
    // init sin_precal
    for (i = 0; i < 10; i++) {
        sin_precal[i] = sin((double)i / 10 * 2 * PI);
    }
