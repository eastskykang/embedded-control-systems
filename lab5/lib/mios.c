...
{
    EMIOS.MCR.B.GPRE = prescaler;                               /* Clock divider , divided by 1 --> max. frequency equal to system clock 120 MHz */
...
void init_PWM(int miosChannel, uint32_t newPeriod_ns)
{
    ...
    EMIOS.CH[miosChannel].CCR.B.UCPREN = 1;                     /* Turn on channel prescaler to have internal clock */
    EMIOS.CH[miosChannel].CCR.B.BSL = 0b11;                     /* Must use internal counter in this mode */
    EMIOS.CH[miosChannel].CCR.B.EDPOL = 0;                      /* Match CADR clears output, match CBDR sets output */
    ...
    SIU.PCR[179 + miosChannel].B.PA = 1;
    SIU.PCR[179 + miosChannel].B.OBE = 1;
}
...
void set_PWMPeriod_ns(int miosChannel, uint32_t newPeriod_ns)
{
    ...
    */
    ...
    EMIOS.CH[miosChannel].CBDR.R = ticks;                       /* set PWM period (ticks)*/
}
...
void set_PWMDutyCycle(int miosChannel, float dutyCycle)
{
    ...
    uint32_t ticks = EMIOS.CH[miosChannel].CBDR.R;

    if (dutyCycle > DutyCycleLimits[MAXIMUM])
        dutyCycle = DutyCycleLimits[MAXIMUM];
    if (dutyCycle < DutyCycleLimits[MINIMUM])
        dutyCycle = DutyCycleLimits[MINIMUM];

    DutyCycle[miosChannel] = dutyCycle;

    /* Write value to eMIOS register here */
    EMIOS.CH[miosChannel].CADR.R = ticks * dutyCycle;
    return;
}
