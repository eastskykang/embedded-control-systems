 ...
 * $Log: etpu_util.c,v $
 * Revision 1.9  2006/12/11 19:12:31  r66544
 * issue:271
 *
 * Preparing for toolbox release 1.0.1. Fixing warnings
 *
 * Revision 1.8  2006/04/18 22:06:30  r61406
 * *** empty log message ***
 *
 * Revision 1.7  2006/04/17 14:05:17  r61406
 * *** empty log message ***
 *
 * Revision 1.6  2006/04/14 20:58:02  r61406
 * *** empty log message ***
 *
...
/******************************************************************************
   ...
   1. Conditionally Load eTPU code into memory.
...
uint32_t fs_etpu_init(struct etpu_config_t p_etpu_config, uint32_t *code,
                      uint32_t code_size, uint32_t *globals, uint32_t
                      globals_size, uint8_t load_microcode)
{
  uint32_t *code_end;
  int32_t unused_code_ram;
  unused_code_ram = (int32_t) (((eTPU->MCR.B.SCMSIZE + 1 ) * 2048) - code_size);
  if (unused_code_ram < 0 )
    return((uint32_t)FS_ETPU_ERROR_CODESIZE);

  /* 1. Conditionally Load microcode */
  if (load_microcode==1) {
  ...
  fs_free_param = fs_memcpy32 ( (uint32_t*)fs_etpu_data_ram_start, globals,
    globals_size);
  return(0);
}

int16_t fs_etpu_chan_init(uint8_t channel, uint8_t function, uint8_t mode, \

uint8_t num_param, uint32_t config, uint16_t func_frame)
{
  int16_t errorCode = 0;
  ...
  /*
     if (func_frame == 0)
     {
     func_frame = mc_etpu_malloc(num_param);

     if (func_frame == 0)
     errorCode = ETPU_ERROR_MALLOC;
     }
   */
  eTPU->CHAN[channel].CR.R |= config + (function<<16) + (func_frame>>3);
...
{
  return((uint32_t*)((uint8_t*)fs_etpu_data_ram_start + (eTPU->CHAN[channel].
           CR.B.CPBA << 3)));
}
...
{
  *(uint32_t *)((uint32_t)fs_etpu_data_ram_start + (eTPU->CHAN[channel].
    CR.B.CPBA<<3) + offset) = value;
}
...
{
  *(uint32_t *)((uint32_t)fs_etpu_data_ram_ext + (eTPU->CHAN[channel].CR.B.CPBA<<
    3) + offset-1) = value;
}
...
{
  *(uint16_t *)((uint32_t)fs_etpu_data_ram_start + (eTPU->CHAN[channel].
    CR.B.CPBA<<3) + offset) = value;
}
...
{
  *(uint8_t *)((uint32_t)fs_etpu_data_ram_start + (eTPU->CHAN[channel].CR.B.CPBA<<
    3) + offset) = value;
}
...
{
  return(*(uint32_t *)((uint32_t)fs_etpu_data_ram_start + (eTPU->CHAN[channel].
           CR.B.CPBA<<3) + offset));
}
...
{
  return((int24_t) (*(uint32_t *)((uint32_t)fs_etpu_data_ram_ext + (eTPU->
            CHAN[channel].CR.B.CPBA<<3) + offset-1)));
}
...
{
  return( 0x00FFFFFF & (*(uint32_t *)((uint32_t)fs_etpu_data_ram_ext +
           (eTPU->CHAN[channel].CR.B.CPBA<<3) + offset-1)));
}
...
{
  return(*(uint16_t *)((uint32_t)fs_etpu_data_ram_start + (eTPU->CHAN[channel].
           CR.B.CPBA<<3) + offset));
}
...
{
  return(*(uint8_t *)((uint32_t)fs_etpu_data_ram_start + (eTPU->CHAN[channel].
           CR.B.CPBA<<3) + offset));
}
...
{
  return((int24_t) (*(uint32_t *)((uint32_t)fs_etpu_data_ram_ext + offset-1)));
}
