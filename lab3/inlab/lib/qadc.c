...
void qadcInit_single(int single_channel)
{
    ...
    /* Disable the FIFO 0 */
    EQADC.CFCR[0].B.MODE = 0b0000;                                      /* disabled */
    ...
    /* Fill in here */
    config.B.HIGH_BYTE = 0b10000000;                                    /* Enable ADC module */
    ...
    /* Fill in here */
    config.B.LOW_BYTE = 0b00000101;                                     /* Use clock prescaler for ADC module */
    ...
    /* Fill in here */
    cmd.B.EOQ = 0b1;                                                    /* End-of-queue True, tells the FIFO to halt after this message, FIFO will still remain in singlecan mode */
    cmd.B.EB = 0b0;                                                     /* Do not use external buffer */
    cmd.B.BN = 0b0;                                                     /* Send commands to ADC0 */
    cmd.B.CAL = 0b0;                                                    /* Do not calibrate the result */
    cmd.B.MESSAGE_TAG = 0b0000;                                         /* Return result to RFIFO RFPR0 */
    cmd.B.LST = 0b10;                                                   /* Sample during 64 clock cycles */
    cmd.B.TSR = 0b0;                                                    /* Do not request a time stamp */
    cmd.B.FMT = 0b0;                                                    /* Conversion data format right justified unsigned */
    cmd.B.CHANNEL_NUMBER = single_channel;                              /* Tell the mux to use the channel which was passed as an argument to the qadcInit_single */
    /* Fill in end */
...
void qadcInit_conti(int channels, uint32_t *inputs)
{
    ...
    /* Disable the FIFO 1 */
    /* Fill in here */;
    EQADC.CFCR[1].B.MODE = 0b0000;                                      /* disabled */
    ...
    for (x = 0; x < channels; x++)
    {
        cmd.R = 0;                                                      /* Clear all bits (VERY IMPORTANT)*/
        /* Fill in here */
        cmd.B.EOQ = 0b0;                                                /* End-of-queue is for software-triggered continuous-scan mode ignored */
        cmd.B.EB = 0b0;         /* Do not use external buffer */
        cmd.B.BN = 0b0;         /* Send commands to ADC0 */
        cmd.B.CAL = 0b0;        /* Do not calibrate the result */
        cmd.B.MESSAGE_TAG = 0b0001;       /* Return result to RFIFO RFPR1 */
        cmd.B.LST = 0b10;       /* Sample during 64 clock cycles */
        cmd.B.TSR = 0b0;        /* Do not request a time stamp */
        cmd.B.FMT = 0b0;        /* Conversion data format right justified unsigned */
        /* Fill in end */
...
vuint16_t qadcReadQ2(vuint8_t channel)
{
    ...
    /* Fill in here */
    return CONT_SCAN_RESULTS[channel];
}
